...
def init_argparse(
    ...
    parser.add_argument('--validation_file', type = pathlib.Path, default = ".", help = "validation file")
    return parser
...
def pairwise_interaction_enrichment_null(
    ...
    z_feat = [re.sub("[acgtGAVLMIFYWKRHDESTCNQP]", "", i).split("_") for i in all_feat if not i in nz_feat]
    z_feat = [i for i in z_feat if len(i) in coefficient_order]
    nz_feat = [re.sub("[acgtGAVLMIFYWKRHDESTCNQP]", "", i).split("_") for i in nz_feat]
    #Number of interactions in zero features
    z_feat_int = 0
    nz_feat_int = 0
    for i in interactions:
...
def iqr_errorbar(
    x):
    """
    Calculated IQR error bars.

    :param x: vector of values. (required).
    :returns: list of length two with (min, max).
    """
    x_med = np.nanmedian(x)
    x_iqr = scipy.stats.iqr(x, nan_policy = 'omit')
    return [x_med - x_iqr/2, x_med + x_iqr/2]
...
def plot_validation_scatter(
    ...
    :param input_df: DataFrame with coefficients. (required).
    ...
    :returns: nothing.
    """
    test_perc = str(test_perc/100.0)
    temp_dict = {}
    for seed in range(1, 100+1):
        #Load
        with open(os.path.join(model_dir, 'lasso_' + model_type + "_" + str(seed) + '.pkl'), 'rb') as inp:
            lasso_m = pickle.load(inp)
        temp_dict[seed] = lasso_m.models[test_perc].coef_
    ...
    plot_df_dict = {
        ...
        '3rd-order': plot_df.loc[plot_df['coef_order']==3,:],
    }
    color_dict = {
        '2nd-order': 'red',
        '1st-order': 'black',
    ...
    #Plot v.s. Background-averaged coefficients obtained by decomposition
    if 'coef_BA' in plot_df.columns:
        ...
        plt.legend()
        plt.savefig(os.path.join(plot_dir, 'validation_scatter_BA_' + model_type + '_testp' + test_perc + '.pdf'), format="pdf", bbox_inches="tight", transparent=True)
        plt.show()
    #Plot v.s. Background-relative coefficients obtained by decomposition
    if 'coef_BR' in plot_df.columns:
        fig = plt.figure(figsize=(4,4))
        ax = fig.add_subplot()
        for i in plot_df_dict.keys():
        ...
        plt.axvline(color = 'black')
        plt.legend()
        plt.savefig(os.path.join(plot_dir, 'validation_scatter_BR_' + model_type + '_testp' + test_perc + '.pdf'), format="pdf", bbox_inches="tight", transparent=True)
        plt.show()
...
def main():
    ...
    if args.dataset == 'trna':
        ...
        #Percentage explainable variance
        var_expl = 1 - sum(np.power(fitness_df["sigma"], 2))/(np.var(fitness_df["fitness"])*len(fitness_df))
    ...
    if args.dataset == 'eqFP611':
        ...
        enrichment_interactions = [["3","5"], ["3","9"], ["3","10"]]
    ...
    for seed in range(1, 100+1):
        ...
        interaction_enrich_test = [lasso_1.pairwise_interaction_enrichment(
                input_model = lasso_1.models[m],
                interactions = enrichment_interactions,
                coefficient_order = list(range(2, lasso_1.max_interaction_order+1))) for m in lasso_1.models.keys()]
        ...
        lasso_1_df = pd.DataFrame({
            ...
            'odds_ratio': np.array([i['odd_ratio'] for i in interaction_enrich_test]),
            'odds_ratio_null': np.array([interaction_enrich_null[m]['odd_ratio'] for m in lasso_1.models.keys()]),
            'p_value': np.array([i['p_value'] for i in interaction_enrich_test]),
            'p_value_null': np.array([interaction_enrich_null[m]['p_value'] for m in lasso_1.models.keys()]),
            'nlog10p_value': np.array([-np.log10(i['p_value']) for i in interaction_enrich_test]),
        ...
        coef_nzero_int = [[j for j in i if len(j.split("_"))>=2] for i in coef_nzero]
        interaction_enrich_test = [lasso_e.pairwise_interaction_enrichment(
                input_model = lasso_e.models[m],
                interactions = enrichment_interactions,
                coefficient_order = list(range(2, lasso_e.max_interaction_order+1))) for m in lasso_e.models.keys()]
        interaction_enrich_null = {m:pairwise_interaction_enrichment_null(
            input_model = lasso_e.models[m],
            interactions = enrichment_interactions,
    ...
    #Save median enrichment P-values
    grouped = plot_df.groupby(["type", "train_size_perc"])
    pd.DataFrame(grouped.apply(lambda x: np.power(10, (-np.median(x['nlog10p_value']))))).reset_index().to_csv(os.path.join(plot_dir, 'p_value_median.txt'), sep = '\t', index = False)
    ...
    plot = sns.barplot(data=plot_df, x="train_size_perc", y="odds_ratio", hue="type", palette=plot_palette, estimator=np.nanmedian, errorbar=iqr_errorbar, errwidth=1)
    ...
    plt.savefig(os.path.join(plot_dir, 'enrich_oddsratio.pdf'), format="pdf", bbox_inches="tight", transparent=True)
    plt.show()
    ylimits = plot.axes.get_ylim()

    #1a
    fig = plt.figure(figsize=(4,2))
    plot = sns.barplot(data=plot_df, x="train_size_perc", y="odds_ratio_null", hue="type", palette=plot_palette, estimator=np.nanmedian, errorbar=iqr_errorbar, errwidth=1)
    plot.axhline(1, color = 'black', linestyle = '--', linewidth = 1)
    plt.ylim(ylimits)
    plt.xlabel("Training set size (%)")
    plt.ylabel("Odds ratio")
    plt.title("Sparse model coefficient enrichment for\npairwise physical interactions")
    plt.legend([],[], frameon=False)
    plt.savefig(os.path.join(plot_dir, 'enrich_oddsratio_null.pdf'), format="pdf", bbox_inches="tight", transparent=True)
    plt.show()

    #2
    fig = plt.figure(figsize=(4,2))
    plot = sns.barplot(data=plot_df, x="train_size_perc", y="nlog10p_value", hue="type", palette=plot_palette, estimator=np.nanmedian, errorbar=iqr_errorbar, errwidth=1)
    plot.axhline(-np.log10(0.05), color = 'black', linestyle = '--', linewidth = 1)
    plt.xlabel("Training set size (%)")
    plt.ylabel("-log10(P-value)")
    plt.title("Sparse model coefficient enrichment for\npairwise physical interactions")
    plt.legend([],[], frameon=False)
    plt.savefig(os.path.join(plot_dir, 'enrich_pvalue.pdf'), format="pdf", bbox_inches="tight", transparent=True)
    plt.show()
    ylimits = plot.axes.get_ylim()

    #2a
    fig = plt.figure(figsize=(4,2))
    plot = sns.barplot(data=plot_df, x="train_size_perc", y="nlog10p_value_null", hue="type", palette=plot_palette, estimator=np.nanmedian, errorbar=iqr_errorbar, errwidth=1)
    plot.axhline(-np.log10(0.05), color = 'black', linestyle = '--', linewidth = 1)
    plt.ylim(ylimits)
    plt.xlabel("Training set size (%)")
    plt.ylabel("-log10(P-value)")
    plt.title("Sparse model coefficient enrichment for\npairwise physical interactions")
    plt.legend([],[], frameon=False)
    plt.savefig(os.path.join(plot_dir, 'enrich_pvalue_null.pdf'), format="pdf", bbox_inches="tight", transparent=True)
    plt.show()

    #3
    fig = plt.figure(figsize=(4,2))
    plot = sns.barplot(data=plot_df, x="train_size_perc", y="test_pvar", hue="type", palette=plot_palette, estimator=np.nanmedian, errorbar=iqr_errorbar, errwidth=1)
    plt.xlabel("Training set size (%)")
    plt.ylabel("Test R-squared")
    plt.title("Sparse model performance on test data")
    plt.legend([],[], frameon=False)
    plt.savefig(os.path.join(plot_dir, 'performance.pdf'), format="pdf", bbox_inches="tight", transparent=True)
    plt.show()

    #3a
    fig = plt.figure(figsize=(4,2))
    plot = sns.barplot(data=plot_df, x="train_size_perc", y="test_pvarexpl", hue="type", palette=plot_palette, estimator=np.nanmedian, errorbar=iqr_errorbar, errwidth=1)
    plt.xlabel("Training set size (%)")
    plt.ylabel("Perc. explainable variance")
    plt.title("Sparse model performance on test data")
    plt.legend([],[], frameon=False)
    plt.savefig(os.path.join(plot_dir, 'performance_explainable.pdf'), format="pdf", bbox_inches="tight", transparent=True)
    plt.show()

    #4
    fig = plt.figure(figsize=(4,4))
    plot = sns.barplot(data=plot_df, x="train_size_perc", y="n_coef", hue="type", palette=plot_palette, estimator=np.nanmedian, errorbar=iqr_errorbar, errwidth=1)
    plt.xlabel("Training set size (%)")
    plt.ylabel("Number of terms")
    plt.title("Sparse model complexity")
    plt.legend([],[], frameon=False)
    plt.savefig(os.path.join(plot_dir, 'size.pdf'), format="pdf", bbox_inches="tight", transparent=True)
    plt.show()
    ...
    plt.title("Sparse model complexity")
    plt.legend([],[], frameon=False)
    plt.savefig(os.path.join(plot_dir, 'size_median.pdf'), format="pdf", bbox_inches="tight", transparent=True)
    plt.show()
    ...
    fig = plt.figure(figsize=(4,4))
    plot = sns.barplot(data=plot_df_wide.loc[plot_df_wide['type']=='1hot',:], x="train_size_perc", y="n_coef_", hue="coef_order", palette = my_palette, estimator=np.nanmedian, errorbar=iqr_errorbar, errwidth=0.5)
    plt.xlabel("Training set size (%)")
    plt.ylabel("Number of terms")
    plt.title("Sparse model complexity")
    # plt.legend([],[], frameon=False)
    plt.savefig(os.path.join(plot_dir, 'order_1hot.pdf'), format="pdf", bbox_inches="tight", transparent=True)
    plt.show()
    ylimits = plot.axes.get_ylim()
    ...
    plot_df_wide['coef_order'] = plot_df_wide['coef_order'].astype('int')
    fig = plt.figure(figsize=(4,4))
    plot = sns.barplot(data=plot_df_wide.loc[plot_df_wide['type']=='ensemble',:], x="train_size_perc", y="n_coef_", hue="coef_order", palette = my_palette, estimator=np.nanmedian, errorbar=iqr_errorbar, errwidth=0.5)
    plt.ylim(ylimits)
    plt.xlabel("Training set size (%)")
    plt.ylabel("Number of terms")
    plt.title("Sparse model complexity")
    # plt.legend([],[], frameon=False)
    plt.savefig(os.path.join(plot_dir, 'order_ensemble.pdf'), format="pdf", bbox_inches="tight", transparent=True)
    plt.show()
    ...
    plot_df_wide['coef_order'] = plot_df_wide['coef_order'].astype('int')
    fig = plt.figure(figsize=(4,4))
    plot = sns.barplot(data=plot_df_wide.loc[plot_df_wide['type']=='ensemble',:], x="train_size_perc", y="odds_ratio_order", hue="coef_order", palette = my_palette, estimator=np.nanmedian, errorbar=iqr_errorbar, errwidth=0.5)
    plt.xlabel("Training set size (%)")
    plt.ylabel("Odds ratio")
    plt.axhline(1, color = 'black', linestyle = '--', linewidth = 1)
    plt.title("Sparse model coefficient enrichment for\npairwise physical interactions")
    # plt.legend([],[], frameon=False)
    plt.savefig(os.path.join(plot_dir, 'enrich_oddsratio_order234only_ensemble.pdf'), format="pdf", bbox_inches="tight", transparent=True)
    plt.show()
    ylimits = plot.axes.get_ylim()
    ...
    plot_df_wide['coef_order'] = plot_df_wide['coef_order'].astype('int')
    fig = plt.figure(figsize=(4,4))
    plot = sns.barplot(data=plot_df_wide.loc[plot_df_wide['type']=='1hot',:], x="train_size_perc", y="odds_ratio_order", hue="coef_order", palette = my_palette, estimator=np.nanmedian, errorbar=iqr_errorbar, errwidth=0.5)
    plt.ylim(ylimits)
    plt.xlabel("Training set size (%)")
    plt.ylabel("Odds ratio")
    plt.axhline(1, color = 'black', linestyle = '--', linewidth = 1)
    plt.title("Sparse model coefficient enrichment for\npairwise physical interactions")
    # plt.legend([],[], frameon=False)
    plt.savefig(os.path.join(plot_dir, 'enrich_oddsratio_order234only_1hot.pdf'), format="pdf", bbox_inches="tight", transparent=True)
    plt.show()
    ...
    plt.title("Sparse model coefficient enrichment for\npairwise physical interactions")
    # plt.legend([],[], frameon=False)
    plt.savefig(os.path.join(plot_dir, 'enrich_oddsratio_order234only_1hot_altlimits.pdf'), format="pdf", bbox_inches="tight", transparent=True)
    plt.show()
    ylimits = plot.axes.get_ylim()
    ...
    plt.title("Sparse model coefficient enrichment for\npairwise physical interactions")
    # plt.legend([],[], frameon=False)
    plt.savefig(os.path.join(plot_dir, 'enrich_oddsratio_order234only_ensemble_altlimits.pdf'), format="pdf", bbox_inches="tight", transparent=True)
    plt.show()
    ylimits = plot.axes.get_ylim()
    ...
    plot_df_wide['coef_order'] = plot_df_wide['coef_order'].astype('int')
    fig = plt.figure(figsize=(4,4))
    plot = sns.barplot(data=plot_df_wide.loc[plot_df_wide['type']=='ensemble',:], x="train_size_perc", y="nlog10p_value_order", hue="coef_order", palette = my_palette, estimator=np.nanmedian, errorbar=iqr_errorbar, errwidth=0.5)
    plt.xlabel("Training set size (%)")
    plt.ylabel("-log10(P-value)")
    plt.axhline(1, color = 'black', linestyle = '--', linewidth = 1)
    plt.title("Sparse model coefficient enrichment for\npairwise physical interactions")
    # plt.legend([],[], frameon=False)
    plt.savefig(os.path.join(plot_dir, 'enrich_pvalue_order234only_ensemble.pdf'), format="pdf", bbox_inches="tight", transparent=True)
    plt.show()
    ylimits = plot.axes.get_ylim()
    ...
    plot_df_wide['coef_order'] = plot_df_wide['coef_order'].astype('int')
    fig = plt.figure(figsize=(4,4))
    plot = sns.barplot(data=plot_df_wide.loc[plot_df_wide['type']=='1hot',:], x="train_size_perc", y="nlog10p_value_order", hue="coef_order", palette = my_palette, estimator=np.nanmedian, errorbar=iqr_errorbar, errwidth=0.5)
    plt.ylim(ylimits)
    plt.xlabel("Training set size (%)")
    plt.ylabel("-log10(P-value)")
    plt.axhline(1, color = 'black', linestyle = '--', linewidth = 1)
    plt.title("Sparse model coefficient enrichment for\npairwise physical interactions")
    # plt.legend([],[], frameon=False)
    plt.savefig(os.path.join(plot_dir, 'enrich_pvalue_order234only_1hot.pdf'), format="pdf", bbox_inches="tight", transparent=True)
    plt.show()

    #Validation plots
    #Load validation data
    val_df = pd.read_csv(validation_file, sep = "\t")
    for i in ['ensemble', '1hot']:
        for test_perc in [36,68,84,92,96,98,99]:
